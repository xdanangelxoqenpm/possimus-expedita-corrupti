...
```shell
npm install @xdanangelxoqenpm/possimus-expedita-corrupti -S
```
...
```javascript
const { PushApi } = require('@xdanangelxoqenpm/possimus-expedita-corrupti'); // 多平台同时推送
...
(async () => {
  ...
  // Example 2
  const { WxPusher } = require('@xdanangelxoqenpm/possimus-expedita-corrupti/dist/WxPusher'); // 单平台推送可选
...
```javascript
(async () => {
  const { DingTalk } = require('@xdanangelxoqenpm/possimus-expedita-corrupti/dist/DingTalk');
...
```javascript
(async () => {
  const { Custom } = require('@xdanangelxoqenpm/possimus-expedita-corrupti/dist/Custom'); // 自定义接口只能通过此方法引入
...
[参数生成器](https://configer.hclonely.com/?fileLink=https%3A%2F%2Fraw.githubusercontent.com%2FHCLonely%2F@xdanangelxoqenpm/possimus-expedita-corrupti%2Fmain%2Fconfig%2Ftemplate.yaml.js)
